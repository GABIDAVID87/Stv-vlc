...
class MainActivity : AppCompatActivity() {
    ...
    private fun recibirProgresoRctv(json: String) {
        try {
            ...
            for (i in 0 until a.length()) {
                ...
                Log.d(
                    "SegoviaRCTV",
                    "Progreso: serie='$seriesTitle' T$season E$episode uri='$uri' pos=$positionMs match=$index"
                )
                if (index >= 0) {
                    val viejo = seguir[index]
                    val serie = if (seriesTitle.isNotBlank()) buscarSerie(seriesTitle) else null
                    val tituloSerie = if (seriesTitle.isNotBlank()) seriesTitle else viejo.titulo
                    val temporadaReal = if (season > 0) season else viejo.temporada
                    val capituloReal = if (episode > 0) episode else viejo.capitulo
                    val tituloEpisodio = if (serie != null && temporadaReal > 0 && capituloReal > 0) {
                        obtenerInfoCapitulo(serie, temporadaReal, capituloReal, uri)?.third?.let {
                            limpiarTituloCapitulo(serie.titulo, it)
                        }.orEmpty()
                    } else ""
                    val subtituloNuevo = if (temporadaReal > 0 && capituloReal > 0) {
                        if (tituloEpisodio.isNotBlank()) "T$temporadaReal - Capítulo $capituloReal - $tituloEpisodio"
                        else "T$temporadaReal - Capítulo $capituloReal"
                    ...
                    seguir[index] = viejo.copy(
                        ...
                        posterUrl = serie?.posterUrl?.takeIf { it.isNotBlank() } ?: viejo.posterUrl,
                        bannerUrl = serie?.bannerUrl?.takeIf { it.isNotBlank() } ?: if (banner.isNotBlank()) banner else viejo.bannerUrl,
                        subtitulo = subtituloNuevo
                    )
                } else if (seriesTitle.isNotBlank() && uri.isNotBlank()) {
                    val serie = buscarSerie(seriesTitle)
                    val temporadaNueva = if (season > 0) season else 0
                    val capituloNuevo = if (episode > 0) episode else 0
                    val episodioNuevo = if (serie != null && temporadaNueva > 0 && capituloNuevo > 0) {
                        obtenerInfoCapitulo(serie, temporadaNueva, capituloNuevo, uri)?.third?.let {
                            limpiarTituloCapitulo(serie.titulo, it)
                        }.orEmpty()
                    } else ""
                    val subtituloNuevo = if (temporadaNueva > 0 && capituloNuevo > 0) {
                        if (episodioNuevo.isNotBlank()) "T$temporadaNueva - Capítulo $capituloNuevo - $episodioNuevo"
                        else "T$temporadaNueva - Capítulo $capituloNuevo"
                    } else ""
                    seguir.add(0, SeguirViendoItem(
                        tipo = "serie",
                        titulo = serie?.titulo ?: seriesTitle,
                        subtitulo = subtituloNuevo,
                        posterUrl = serie?.posterUrl?.takeIf { it.isNotBlank() } ?: poster,
                        bannerUrl = serie?.bannerUrl?.takeIf { it.isNotBlank() } ?: banner,
                        sinopsis = "",
                        streamUrl = uri,
                        temporada = temporadaNueva,
    ...
    private fun actualizarProgresoDesdeRctv() {
        ...
        val normalizedUri = normalizeProgressUrl(uri)
        val index = seguir.indexOfFirst { normalizeProgressUrl(it.streamUrl) == normalizedUri }
    ...
    private fun guardarCapitulo(s: SerieDrive, t: Temporada, n: Int, c: Capitulo) {
        ...
        val normalized = normalizeProgressUrl(c.streamUrl)
        val progresoAnterior = seguir.firstOrNull {
            it.tipo == "serie" &&
                (normalizeProgressUrl(it.streamUrl) == normalized ||
    ...
    private fun buscarSerie(titulo: String): SerieDrive? {
        ...
        val encontrada = repo.series.firstOrNull { it.titulo.equals(titulo, ignoreCase = true) }
    ...
    private fun obtenerInfoCapitulo(
        ...
        val temporadaObj = serie.temporadas.firstOrNull { it.numero == temporada } ?: return null
        if (capitulo !in 1..temporadaObj.capitulos.size) return null
        val indice = capitulo - 1
        val candidato = temporadaObj.capitulos[indice]
        if (streamUrl.isBlank()) return Triple(temporadaObj, capitulo, candidato)
        if (normalizeProgressUrl(candidato.streamUrl) == normalizeProgressUrl(streamUrl)) {
            return Triple(temporadaObj, capitulo, candidato)
        }
        val porUrl = temporadaObj.capitulos.indexOfFirst {
            normalizeProgressUrl(it.streamUrl) == normalizeProgressUrl(streamUrl)
        }
        return if (porUrl >= 0) Triple(temporadaObj, porUrl + 1, temporadaObj.capitulos[porUrl]) else null
    }
    ...
    private fun repararSeguirViendo() {
        ...
        seguir.forEachIndexed { index, item ->
            ...
            val info = obtenerInfoCapitulo(serie, item.temporada, item.capitulo, normalizeProgressUrl(item.streamUrl))
            ...
            val nuevoPoster = serie.posterUrl.ifBlank { item.posterUrl }
            val nuevoBanner = serie.bannerUrl.ifBlank { item.bannerUrl }
            val nuevaUrl = if (item.streamUrl.isNotBlank()) item.streamUrl else info?.third?.streamUrl.orEmpty()
            val nuevo = item.copy(
                titulo = serie.titulo,
                subtitulo = nuevoSubtitulo,
                posterUrl = nuevoPoster,
                bannerUrl = nuevoBanner,
                streamUrl = nuevaUrl
            )
            if (nuevo != item) {
                seguir[index] = nuevo
