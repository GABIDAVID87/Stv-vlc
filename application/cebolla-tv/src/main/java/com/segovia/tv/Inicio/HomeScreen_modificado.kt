...
class HomeScreen(
    ...
    fun show() {
        ...
        val episodeInfo = TextView(activity).apply {
            text = ""
            textSize = 22f
        ...
        tc.addView(title, FrameLayout.LayoutParams(-1, -2).apply {
            leftMargin = 0
            topMargin = 0
        })

        tc.addView(episodeInfo, FrameLayout.LayoutParams(-1, -2).apply {
            leftMargin = 0
            topMargin = dp(46)
        })

        tc.addView(synopsis, FrameLayout.LayoutParams(-1, -2).apply {
            leftMargin = 0
            topMargin = dp(75)
        })
        ...
        fun updateContinue(item: SeguirViendoItem) {
            ...
            val sinopsis = item.sinopsis
            val banner = item.bannerUrl

            title.text = name(titulo)

            if (item.tipo == "serie") {
                var info = item.subtitulo.trim()

                if (info.isBlank() && item.temporada > 0 && item.capitulo > 0) {
                    info = "T${item.temporada} - Capítulo ${item.capitulo}"
                }

                episodeInfo.text = info
                episodeInfo.visibility = if (info.isNotBlank()) View.VISIBLE else View.GONE
            } else {
                episodeInfo.text = ""
                episodeInfo.visibility = View.GONE
            }
        ...
        root.post {
            ...
            tc.layoutParams =
                FrameLayout.LayoutParams(
                    ...
                    topMargin = (h * 0.405f).toInt() - dp(20)
                }
