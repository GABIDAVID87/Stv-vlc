...
class MainActivity : AppCompatActivity() {
    ...
    private val connection = object : MediaBrowserCompat.ConnectionCallback() {
        ...
        override fun onConnectionSuspended() { polling = false }
        override fun onConnectionFailed() { polling = false }
    }

    private val controllerCallback = object : MediaControllerCompat.Callback() {
        override fun onPlaybackStateChanged(state: PlaybackStateCompat?) { capturarProgreso(true) }
        override fun onMetadataChanged(metadata: MediaMetadataCompat?) { capturarProgreso(true) }
        override fun onQueueChanged(queue: MutableList<MediaSessionCompat.QueueItem>?) { capturarProgreso(true) }
    }
    ...
    private fun normalizeUrl(raw: String): String {
        ...
        if (u.startsWith("video?")) return "https://segovia-tv-proxy.guadianesgalaxi.workers.dev/$u"
        if (u.startsWith("undefined/video?", true)) return "https://segovia-tv-proxy.guadianesgalaxi.workers.dev/" + u.substringAfter("undefined/")
        if (u.startsWith("null/video?", true)) return "https://segovia-tv-proxy.guadianesgalaxi.workers.dev/" + u.substringAfter("null/")
        return u
    }
    ...
    private fun guardarProgreso(url: String, position: Long, duration: Long) {
        ...
        val item = SeguirViendoItem("serie", s.titulo, "${t.titulo} • Capítulo $n", s.posterUrl, s.bannerUrl, cap.sinopsis, cap.streamUrl, t.numero, n, progreso.coerceAtMost(Int.MAX_VALUE.toLong()).toInt())
    ...
    private fun guardarLista() {
        val a = JSONArray()
        seguir.forEach { a.put(JSONObject().apply {
            put("tipo", it.tipo); put("titulo", it.titulo); put("subtitulo", it.subtitulo); put("posterUrl", it.posterUrl); put("bannerUrl", it.bannerUrl); put("sinopsis", it.sinopsis); put("streamUrl", it.streamUrl); put("temporada", it.temporada); put("capitulo", it.capitulo); put("progreso", it.progreso)
        }) }
    ...
    private fun openContinue(item: SeguirViendoItem) {
        if (item.tipo == "serie") {
            repo.series.firstOrNull { it.titulo.equals(item.titulo, true) }?.let {
                serieSeleccionada = it
        ...
        repo.peliculas.firstOrNull { it.streamUrl == item.streamUrl }?.let {
            peliculaSeleccionada = it
            previousScreen = currentScreen
    ...
    private fun openMovie(p: PeliculaDrive) { peliculaSeleccionada = p; serieSeleccionada = null; previousScreen = currentScreen; currentScreen = "DetallesPeli"; movieDetails.show(p) }
    private fun openSeries(s: SerieDrive) { serieSeleccionada = s; peliculaSeleccionada = null; previousScreen = currentScreen; currentScreen = "DetallesSerie"; series.showDetail(s) }
