...
class MainActivity : AppCompatActivity() {
    ...
    private fun iniciarMediaSessionRctv() {
        ...
        for (pkg in rctvPackages) {
            try {
                val browser = MediaBrowserCompat(this, ComponentName(this, rctvServiceName), rctvConnectionCallback, null)
                rctvBrowser = browser
                browser.connect()
    ...
    private fun actualizarProgresoDesdeRctv() {
        ...
        val index = seguir.indexOfFirst { normalizeProgressUrl(it.streamUrl) == normalizeProgressUrl(uri) }
