...
class PlaylistManager(val service: PlaybackService) : MediaWrapperList.EventListener, IMedia.EventListener, CoroutineScope {
            ...
            when (event.type) {
                MediaPlayer.Event.Playing -> {
                    ...
                    if (newMedia) {
                        ...
                        // SEGOVIA TV DIAGNOSTIC:
                        // Log the title immediately before and after VLC metadata refresh.
                        // This does not change playback, progress, history or metadata behavior.
                        if (BuildConfig.DEBUG) {
                            Log.d("SEGOVIA_TITLE_DEBUG", "BEFORE_REFRESH | title=${mw.title} | fileName=${mw.fileName} | location=${mw.location} | playerTitle=${player.mediaplayer.media?.getMeta(IMedia.Meta.Title, true)}")
                        }
                        ...
                        if (mw.title == mw.fileName || (mw.type == MediaWrapper.TYPE_STREAM && (mw.title != player.mediaplayer.media?.getMeta(IMedia.Meta.Title, true) || mw.artistName != player.mediaplayer.media?.getMeta(IMedia.Meta.Artist, true)))) {
                            ...
                            if (BuildConfig.DEBUG) {
                                Log.d("SEGOVIA_TITLE_DEBUG", "AFTER_REFRESH | title=${mw.title} | fileName=${mw.fileName} | location=${mw.location} | playerTitle=${player.mediaplayer.media?.getMeta(IMedia.Meta.Title, true)}")
                            }
                        } else if (BuildConfig.DEBUG) {
                            Log.d("SEGOVIA_TITLE_DEBUG", "NO_REFRESH | title=${mw.title} | fileName=${mw.fileName} | location=${mw.location} | playerTitle=${player.mediaplayer.media?.getMeta(IMedia.Meta.Title, true)}")
                        }
                    }
     ...
     */
    private fun isSegoviaTvExplicitTitle(title: String?): Boolean {
        if (title.isNullOrBlank()) return false
        return Regex("""^.+\s-\sT\d+\s-\sCap[ií]tulo\s+\d+(?:\s-\s.*)?$""", RegexOption.IGNORE_CASE).matches(title.trim())
    }

    private fun refreshTrackMeta(mw: MediaWrapper) {
        // Segovia TV supplies an explicit chapter title. VLC's stream metadata
        // can report the URL/query as the title; do not let that metadata replace
        // the title supplied by Segovia when this is one of our chapter titles.
        val explicitSegoviaTitle = mw.title?.toString()?.takeIf { isSegoviaTvExplicitTitle(it) }

        mw.updateMeta(player.mediaplayer)

        if (explicitSegoviaTitle != null) {
            mw.title = explicitSegoviaTitle
            mw.setDisplayTitle(explicitSegoviaTitle)
        }
