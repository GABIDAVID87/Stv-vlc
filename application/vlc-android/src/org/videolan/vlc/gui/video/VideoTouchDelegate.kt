...
// Mouse events for the core
    ...
    private fun doSeekTouch(coef: Int, gesturesize: Float, seek: Boolean) {
        if (touchControls and TOUCH_FLAG_SWIPE_SEEK != 0 && initInAllowedBounds) {
            ...
            // SEGOVIA TV: perform the seek without showing the +10s / -10s
            // jump indicator. The seek operation itself is preserved.
        }
    }
    ...
    // SEGOVIA TV: keep a stable position during rapid consecutive seeks.
    // PlaybackService may not have updated getTime() yet when the next button
    // press arrives, so each seek is based on the previous requested position.
    private var rapidSeekPosition = -1L
    private var rapidSeekTimestamp = 0L
    private val rapidSeekResetRunnable = Runnable { rapidSeekPosition = -1L }
    ...
    internal fun seekDelta(delta: Int) {
        player.service?.let { service ->
            ...
            val now = System.currentTimeMillis()
            val basePosition = if (rapidSeekPosition >= 0L && now - rapidSeekTimestamp <= SEEK_TIMEOUT) {
                rapidSeekPosition
            } else {
                player.time
            }

            val position = (basePosition + delta).coerceIn(0L, service.length)

            rapidSeekPosition = position
            rapidSeekTimestamp = now
            handler.removeCallbacks(rapidSeekResetRunnable)
            handler.postDelayed(rapidSeekResetRunnable, SEEK_TIMEOUT)

            // SEGOVIA TV: perform the seek but do not display the visual seek
            // overlay (+10s / -10s) in the upper corners.
            // The playback position change itself is preserved.
