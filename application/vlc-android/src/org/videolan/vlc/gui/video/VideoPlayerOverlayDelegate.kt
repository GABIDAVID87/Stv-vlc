...
class VideoPlayerOverlayDelegate (private val player: VideoPlayerActivity) {
    ...
    // Segovia TV: cuando la playlist lateral está abierta, debe permanecer visible
    // aunque VLC oculte los controles al comenzar la reproducción.
    private var segoviaPlaylistPinned = false
    ...
    private fun resetHingeLayout() {
        // La playlist de Segovia es un panel lateral fijo a la derecha.
        // No cambiar su ancho a MATCH_PARENT al restaurar el layout normal.
        ...
        if (::playlistContainer.isInitialized &&
            player.intent.getBooleanExtra("from_external", false) &&
            player.intent.getStringExtra("content_type").equals("series", ignoreCase = true)) {
            val lp = playlistContainer.layoutParams
            lp.width = player.resources.displayMetrics.density.let { density ->
                (430f * density).toInt()
            }
            playlistContainer.layoutParams = lp
        }
        arrayOf(playerUiContainer, playlistContainer).forEach {
    ...
    private fun setupSegoviaPlaylistHeader() {
        ...
        val season = player.intent.getStringExtra("season")?.trim().orEmpty()
            .ifBlank {
                player.intent.getIntExtra("segovia_season", -1)
                    .takeIf { it > 0 }
                    ?.toString()
                    .orEmpty()
            }
        val bannerUrl = player.intent.getStringExtra("segovia_banner_url")?.trim().orEmpty()
        val footer = player.findViewById<TextView>(R.id.segovia_playlist_footer)
        ...
        player.lifecycleScope.launch(Dispatchers.IO) {
            ...
            val bitmap = try {
                connection = URL(bannerUrl).openConnection() as HttpURLConnection
            ...
            withContext(Dispatchers.Main) {
                if (!player.isFinishing && bitmap != null) {
    ...
    fun togglePlaylist() {
        ...
        if (player.isPlaylistVisible) {
            if (isSegoviaSeries) segoviaPlaylistPinned = false
        ...
        if (isSegoviaSeries) segoviaPlaylistPinned = true
        hideOverlay(true)
    ...
    fun showControls(show: Boolean) {
        ...
        // SEGOVIA TV: la playlist lateral no forma parte del HUD que VLC oculta
        // automáticamente al comenzar la reproducción. Si estaba abierta,
        // la mantenemos visible. El botón de cerrar sigue pudiendo ocultarla.
        // SEGOVIA TV: la playlist de una serie externa es un panel fijo.
        // No dependemos solamente de segoviaPlaylistPinned porque el flujo
        // de reproducción puede reconstruir/actualizar el Intent antes de
        // ocultar los controles.
        val isSegoviaSeries = player.intent.getBooleanExtra("from_external", false) &&
                (player.intent.getStringExtra("content_type").equals("series", ignoreCase = true) ||
                        !player.intent.getStringExtra("series_title").isNullOrBlank())
        if (::playlistContainer.isInitialized && (segoviaPlaylistPinned || isSegoviaSeries)) {
            playlistContainer.setVisible()
        }
