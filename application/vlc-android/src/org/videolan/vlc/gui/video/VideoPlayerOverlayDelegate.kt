...
class VideoPlayerOverlayDelegate (private val player: VideoPlayerActivity) {
    ...
    /**
     * SEGOVIA TV: keeps the custom series playlist visible while VLC starts/switches
     * the current media. The normal VLC HUD may hide itself, but this side panel
     * must remain on screen until the user closes it.
     */
    fun keepSegoviaPlaylistVisible() {
        val isSegoviaSeries = player.intent.getBooleanExtra("from_external", false) &&
                player.intent.getStringExtra("content_type").equals("series", ignoreCase = true)
        if (!isSegoviaSeries) return
        segoviaPlaylistPinned = true
        if (::playlistContainer.isInitialized) {
    ...
    fun showControls(show: Boolean) {
        ...
        if (::playlistContainer.isInitialized && segoviaPlaylistPinned) {
            playlistContainer.setVisible()
        }
