...
class PlaylistAdapter(private val player: IPlayer) : DiffUtilAdapter<MediaWrapper, PlaylistAdapter.ViewHolder>(), SwipeDragHelperAdapter, SchedulerCallback {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // SEGOVIA TV: el rectángulo amarillo pertenece SOLO al foco del mando.
        // No usamos selected para marcar el episodio que se está reproduciendo.
        ...
        // SEGOVIA TV: el episodio activo queda amarillo, pero sin rectángulo.
        // El rectángulo amarillo lo controla exclusivamente el foco del mando.
        ...
        holder.binding.selector.setOnFocusChangeListener { _, hasFocus ->
            holder.binding.audioItemTitle.setTextColor(
                if (hasFocus || currentIndex == position) 0xFFFFD700.toInt()
                else defaultTitleColor
            )
        }
        holder.binding.audioItemTitle.setTextColor(
            if (holder.binding.selector.hasFocus() || currentIndex == position) 0xFFFFD700.toInt()
            else defaultTitleColor
        )
    }
