...
class PlaylistAdapter(private val player: IPlayer) : DiffUtilAdapter<MediaWrapper, PlaylistAdapter.ViewHolder>(), SwipeDragHelperAdapter, SchedulerCallback {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // SEGOVIA TV: el rectángulo amarillo representa SOLO el foco del mando.
        // No usamos selected para marcar el capítulo que se está reproduciendo,
        // porque eso dejaría el rectángulo pegado al capítulo actual.
        holder.binding.selector.isSelected = false

        // El foco se mueve con el D-pad. Solo el título recibe el amarillo cuando
        // tiene el foco o cuando es el capítulo que se está reproduciendo.
        val defaultTitleColor = holder.binding.audioItemTitle.textColors.defaultColor
        holder.binding.selector.setOnFocusChangeListener { _, hasFocus ->
            holder.binding.audioItemTitle.setTextColor(if (hasFocus || currentIndex == position) 0xFFFFD700.toInt() else defaultTitleColor)
        }
        holder.binding.audioItemTitle.setTextColor(if (holder.binding.selector.hasFocus() || currentIndex == position) 0xFFFFD700.toInt() else defaultTitleColor)
