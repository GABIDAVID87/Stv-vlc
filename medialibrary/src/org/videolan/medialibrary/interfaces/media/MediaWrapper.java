...
public abstract class MediaWrapper extends MediaLibraryItem implements Parcelable {
    ...
    private String getMetaTitle(IMedia media) {
        ...
        if (TextUtils.isEmpty(libvlcTitle))
            return getTitle();

        // For streams, VLC can return the stream URL (or the path/query
        // portion of it) as the media title. In that case keep the
        // application-provided title instead.
        if (media.getType() == Media.Type.Stream) {
            String mediaUri = media.getUri();

            if (!TextUtils.isEmpty(mediaUri)) {
                String normalizedUri = mediaUri.trim();
                String normalizedTitle = libvlcTitle.trim();

                // Title is exactly the URL.
                if (normalizedTitle.equals(normalizedUri))
                    return getTitle();

                // Title is only the path/query portion of the URL.
                try {
                    Uri uri = Uri.parse(normalizedUri);
                    String pathAndQuery = uri.getPath();

                    if (!TextUtils.isEmpty(pathAndQuery)) {
                        String query = uri.getQuery();
                        if (!TextUtils.isEmpty(query))
                            pathAndQuery += "?" + query;

                        if (normalizedTitle.equals(pathAndQuery))
                            return getTitle();
                    }
            ...
            // Keep the original VLC protection for URL-like stream titles.
            if (libvlcTitle.toLowerCase(Locale.ENGLISH).contains("://"))
                return getTitle();
        }
