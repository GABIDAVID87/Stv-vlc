...
public abstract class MediaWrapper extends MediaLibraryItem implements Parcelable {
    ...
    private String getMetaTitle(IMedia media) {
        ...
        // For streams, VLC can return the stream URL (or only the
        // path/query part of it) as the media title. Keep the title
        // already supplied by the application in that case.
        if (media.getType() == Media.Type.Stream) {
            String normalizedTitle = libvlcTitle.trim();
            String mediaLocation = mUri != null ? mUri.toString().trim() : "";

            if (!TextUtils.isEmpty(mediaLocation)) {
                if (normalizedTitle.equals(mediaLocation) ||
                        mediaLocation.endsWith("/" + normalizedTitle) ||
                        mediaLocation.endsWith(normalizedTitle)) {
                    return getTitle();
                }
            }
            ...
            if (normalizedTitle.toLowerCase(Locale.ENGLISH).contains("://"))
                return getTitle();
        }
